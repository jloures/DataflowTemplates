...
public class BigtableUtilsTest {
  ...
  /**
   * Test whether {@link BigtableUtils} can create {@link com.google.cloud.teleport.bigtable.BigtableRow}
   * objects appropriately from a {@link com.google.cloud.teleport.bigtable.ChangelogEntry} of type
   * SET_CELL.
   */
  @Test
  public void testCreateBigtableRowSetCellEntry() {
    ...
    HashMap<ByteBuffer, ByteBuffer> changelogEntryHashMap = getHashMapFromChangelogEntry(
        setCellEntry, CHARSET);

    ByteBuffer bigtableRowExpectedRowKey = createChangelogRowKey(utils,
        setCellEntry.getCommitTimestamp(), WORKER_ID, COUNTER, CHARSET);
  ...
  public void testCreateBigtableRowDeleteCellsEntry() {
    ...
    ChangelogEntry deleteCellsEntry = createChangelogEntry(ModType.DELETE_CELLS, COLUMN_FAMILY, COLUMN);

    BigtableRow deleteCellsRow = utils.createBigtableRow(deleteCellsEntry, WORKER_ID,
        COUNTER);
    HashMap<ByteBuffer, ByteBuffer> changelogEntryHashMap = getHashMapFromChangelogEntry(
        deleteCellsEntry, CHARSET);

    ByteBuffer bigtableRowExpectedRowKey = createChangelogRowKey(utils,
        deleteCellsEntry.getCommitTimestamp(), WORKER_ID, COUNTER, CHARSET);
  ...
  public void testCreateBigtableRowDeleteFamilyEntry() {
    ...
    ChangelogEntry deleteFamilyEntry = createChangelogEntry(ModType.DELETE_FAMILY, COLUMN_FAMILY, COLUMN);

    BigtableRow deleteFamilyRow = utils.createBigtableRow(deleteFamilyEntry, WORKER_ID, COUNTER);
    HashMap<ByteBuffer, ByteBuffer> changelogEntryHashMap = getHashMapFromChangelogEntry(
        deleteFamilyEntry, CHARSET);

    ByteBuffer bigtableRowExpectedRowKey = createChangelogRowKey(utils,
        deleteFamilyEntry.getCommitTimestamp(), WORKER_ID, COUNTER, CHARSET);
  ...
  public void testGetValidEntriesAllEntriesAreValid() {
    ...
    Mockito.when(mutation.getEntries()).thenReturn(expectedEntries);
  ...
  private HashMap<ByteBuffer, ByteBuffer> getHashMapFromChangelogEntry(ChangelogEntry entry,
      Charset charset) {
  ...
  private void addCommonEntryProperties(HashMap<ByteBuffer, ByteBuffer> entryMap,
      ChangelogEntry entry, Charset charset) {
    entryMap.put(ChangelogColumns.ROW_KEY.getColumnNameAsByteBuffer(charset),
        BigtableUtils.copyByteBuffer(entry.getRowKey()));
    entryMap.put(ChangelogColumns.MOD_TYPE.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getModType().toString(), charset));
    entryMap.put(ChangelogColumns.IS_GC.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getIsGc().toString(), charset));
    entryMap.put(ChangelogColumns.TIEBREAKER.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getTieBreaker().toString(), charset));
    entryMap.put(ChangelogColumns.COMMIT_TIMESTAMP.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getCommitTimestamp().toString(), charset));
    entryMap.put(ChangelogColumns.LOW_WATERMARK.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getLowWatermark().toString(), charset));
  }

  private void addSetCellEntryProperties(HashMap<ByteBuffer, ByteBuffer> entryMap,
      ChangelogEntry entry, Charset charset) {
    entryMap.put(ChangelogColumns.COLUMN_FAMILY.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getColumnFamily().toString(), charset));
    entryMap.put(ChangelogColumns.COLUMN.getColumnNameAsByteBuffer(charset),
        BigtableUtils.copyByteBuffer(entry.getColumn()));
    entryMap.put(ChangelogColumns.TIMESTAMP.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getTimestamp().toString(), charset));
    entryMap.put(ChangelogColumns.VALUE.getColumnNameAsByteBuffer(charset),
  ...
  private void addDeleteCellsEntryProperties(HashMap<ByteBuffer, ByteBuffer> entryMap,
      ChangelogEntry entry, Charset charset) {
    entryMap.put(ChangelogColumns.COLUMN_FAMILY.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getColumnFamily().toString(), charset));
    entryMap.put(ChangelogColumns.COLUMN.getColumnNameAsByteBuffer(charset),
    ...
    entryMap.put(ChangelogColumns.TIMESTAMP_TO.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getTimestampTo().toString(), charset));
    entryMap.put(ChangelogColumns.TIMESTAMP_FROM.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getTimestampFrom().toString(), charset));
  }

  private void addDeleteFamilyEntryProperties(HashMap<ByteBuffer, ByteBuffer> entryMap,
      ChangelogEntry entry, Charset charset) {
    entryMap.put(ChangelogColumns.COLUMN_FAMILY.getColumnNameAsByteBuffer(charset),
        getByteBufferFromString(entry.getColumnFamily().toString(), charset));
  ...
  private BigtableUtils initBigtableUtils(String ignoreColumnFamilies,
  ...
  private ChangelogEntry createChangelogEntry(ModType modType, String columnFamily,
  ...
  private ChangelogEntry createSetCellChangelogEntry(String columnFamily,
      ByteBuffer qualifier) {
  ...
  private ChangelogEntry createDeleteCellsChangelogEntry(String columnFamily,
      ByteBuffer qualifier) {
    return ChangelogEntry.newBuilder()
        .setRowKey(ROW_KEY)
        .setIsGc(IS_GC)
  ...
  private ChangelogEntry createDeleteFamilyChangelogEntry(String columnFamily,
      ByteBuffer qualifier) {
    return ChangelogEntry.newBuilder()
        .setRowKey(ROW_KEY)
        .setIsGc(IS_GC)
